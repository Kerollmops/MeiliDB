...
fn compact_index(db_path: PathBuf, index_name: &str) -> anyhow::Result<()> {
    ...
    for result in index_mapping.iter(&rtxn)? {
        ...
        let compacted_index_file = std::fs::File::create_new(&compacted_index_file_path)
            .with_context(|| {
                format!(
                    "While creating a new file at {}. Please delete this file before continuing.",
                    compacted_index_file_path.display()
                )
            })?;
        drop(compacted_index_file);
